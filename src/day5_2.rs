...
pub fn main() {
    ...
    println!("Part 1: {} in {}ms", result_1, dur_part_1.as_millis());

    let start_part_2 = Instant::now();

    let mut positions_2 = (0..9).map(|i| Position::new(i, 0)).collect::<Vec<Position>>();

    //Do the crane ops in reverse
    reversed_flipped_crane_ops.iter().for_each(|op| {
        positions_2.iter_mut().for_each(|p| {
            p.traceback_op(op, false);
        });
    });

    let dur_part_2 = start_part_2.elapsed();

    let result_2 : String = positions_2.iter()
        .map(|p| {
            let crate_stack = &crate_stacks[p.stack_index];
            let crate_char = crate_stack.crates[crate_stack.crates.len() - 1 - p.n_crates_on_top];
            crate_char
    ...
    println!("Part 2: {} in {}ms", result_2, dur_part_2.as_millis());
}
...
impl Position {
    ...
    pub fn traceback_op(&mut self, op: &CraneOp, reverse: bool) {
