...
pub fn main(){
    ...
    let x_vec = cpu.run(&instructions);

    dbg!(&x_vec);
    let part_1 = [20,60,100,140,180,220].iter().map(|i| x_vec[*i] * (*i as i32)).sum::<i32>();
    println!("Part 1: {}", part_1);

    println!("Part 2:");
    let x_vec_trimmed = x_vec.iter().skip(1).collect::<Vec<&i32>>();
        ...
        loop {
            ...
            match &self.state{
                ...
                CPUState::Executing(op, cycles_left) => {
                    self.state = CPUState::Executing(op.clone(), cycles_left - 1);
                }
            }
            self.cycle += 1;
        }
