...
fn main() {
    ...
    let sum_misplaced_items = rucksacks.iter()
        .map(|r| r.item_in_c1_and_c2.unwrap())
        .sum::<u32>();
...
struct Rucksack {
    ...
    pub item_in_c1_and_c2: Option<u32>
}
...
impl FromStr for Rucksack {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let mut item_in_c1 = [false; 53];
        let mut item_in_c2 = [false; 53];

        s1.chars().for_each(|c| item_in_c1[(item_value_mapper(c))] = true);
        s2.chars().for_each(|c| item_in_c2[(item_value_mapper(c))] = true);

        let item_in_c1_and_c2 = item_in_c1.iter().zip(item_in_c2.iter()).zip(0..53)
            .find(|((a,b), i)| **a && **b)
            .map(|(_, i)| i);
        ...
        Ok(Rucksack { item_present, item_in_c1_and_c2})
    }
}
